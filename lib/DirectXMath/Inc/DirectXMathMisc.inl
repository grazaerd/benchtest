...
) noexcept
{
    ...
    // Map Value to y in [-pi,pi], x = 2*pi*quotient + remainder.
    float q = roundf(Value * XM_1DIV2PI);        // nearest integer
    float y = Value - XM_2PI * q;

    // 2) Fold into [–π/2, π/2] branchlessly
    //    c1 = 1.0 if y > π/2, else 0.0
    //    c2 = 1.0 if y < -π/2, else 0.0
    float c1 = (y >  XM_PIDIV2) ? 1.0f : 0.0f;
    float c2 = (y < -XM_PIDIV2) ? 1.0f : 0.0f;

    // If c1: newY = π - y, if c2: newY = -π - y
    // y + c1*(newY - y) + c2*(newY2 - y)
    y = y 
      + c1 * (( XM_PI  - y) - y)
      + c2 * ((-XM_PI - y) - y);

    // 3) Compute cosine‐sign = +1 inside [–π/2,π/2], −1 otherwise
    //    (c1 + c2) is either 0 or 1
    float signCos = 1.0f - 2.0f * (c1 + c2);

    // 4) Polynomial approximations (same minimax as before)
    float y2 = y * y;

    *pSin = (((-0.00018363653976946785f * y2
            + 0.008306325227159894f) * y2
            - 0.16664828381895057f) * y2
            + 1.0f) * y;

    float p = (((-0.0012712094856965508f * y2
                + 0.04148774804542921f) * y2
                - 0.4999124397122458f) * y2
            + 1.0f);
    *pCos = signCos * p;
}
