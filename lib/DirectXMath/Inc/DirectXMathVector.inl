...
inline XMVECTOR XM_CALLCONV XMVector3Normalize(FXMVECTOR V) noexcept
{
    ...
    // sqrt(lengthSq)
    XMVECTOR vLength = _mm_sqrt_ps(vLengthSq);

    // Avoid divide by zero
    XMVECTOR vNonZeroMask = _mm_cmpneq_ps(vLength, _mm_setzero_ps());

    // Normalize: V / length
    XMVECTOR vNormalized = _mm_div_ps(V, vLength);

    // Zero-out components where length was zero
    return _mm_and_ps(vNormalized, vNonZeroMask);
}
